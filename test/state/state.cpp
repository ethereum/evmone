    ...
    int64_t blob_gas_left) noexcept
{
    ...
    case Transaction::Type::blob:
        ...
        if (tx.max_blob_gas_price < compute_blob_gas_price(block.excess_blob_gas))
    ...
    int64_t execution_gas_limit)
{
    ...
    if (tx.type == Transaction::Type::blob)
    {
        const auto blob_fee = tx.blob_gas_used() * compute_blob_gas_price(block.excess_blob_gas);
