...
namespace
{
TransitionResult apply_block(MegaContext& mega_ctx, TestState& state, const state::BlockInfo& block,
    const std::vector<state::Transaction>& txs, evmc_revision rev,
    std::optional<int64_t> block_reward)
{
    const auto diff = state::system_call(mega_ctx, state, block, rev);
    ...
    for (size_t i = 0; i < txs.size(); ++i)
    {
        ...
        auto res = test::transition(mega_ctx, state, block, tx, rev, block_gas_left, blob_gas_left);
...
void run_blockchain_tests(std::span<const BlockchainTest> tests, MegaContext& mega_ctx)
{
    for (size_t case_index = 0; case_index != tests.size(); ++case_index)
    {
        ...
        const auto genesis_res =
            apply_block(mega_ctx, state, genesis, {}, c.rev.get_revision(0), {});
        ...
        for (const auto& test_block : c.test_blocks)
        {
            ...
            const auto res =
                apply_block(mega_ctx, state, bi, test_block.transactions, rev, mining_reward(rev));
