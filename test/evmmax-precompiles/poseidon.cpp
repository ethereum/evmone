...
#define POW5MODX(SLOT_IDX, TMP_SLOT)              \
    res += mulmodx(TMP_SLOT, SLOT_IDX, SLOT_IDX); \
    res += mulmodx(TMP_SLOT, TMP_SLOT, TMP_SLOT); \
    res += mulmodx(SLOT_IDX, SLOT_IDX, TMP_SLOT);

bytecode create_poseidon_hash_bytecode()
{
    bytecode res;
    size_t mem_ptr = 0;
    ...
    // Store MDS matrix
    const size_t mds_prt = mem_ptr;
    for (size_t i = 0; i < 9; ++i)
    {
        res += mstore(mem_ptr, push(M[i]));
        mem_ptr += 32;
    }

    const uint8_t mds_slot_start = 0;
    const uint8_t M00_slot = mds_slot_start;
    const uint8_t M01_slot = M00_slot + 1;
    const uint8_t M02_slot = M01_slot + 1;
    const uint8_t M10_slot = M02_slot + 1;
    const uint8_t M11_slot = M10_slot + 1;
    const uint8_t M12_slot = M11_slot + 1;
    const uint8_t M20_slot = M12_slot + 1;
    const uint8_t M21_slot = M20_slot + 1;
    const uint8_t M22_slot = M21_slot + 1;

    res += storex(9, mds_prt, mds_slot_start);
    const uint8_t mds_slot_end = mds_slot_start + 9;

    const uint8_t state_slot_start = mds_slot_end;
    const uint8_t state_slot_end = state_slot_start + 3;
    res += calldatacopy(mem_ptr, 0, 64);

    const uint8_t swap_slot_start = state_slot_end;
    const uint8_t swap_slot_end = swap_slot_start + 3;

    res += storex(2, mem_ptr, swap_slot_start + 1);
    mem_ptr += 64;

    const uint8_t state0_slot = state_slot_start;
    const uint8_t state1_slot = state0_slot + 1;
    const uint8_t state2_slot = state1_slot + 1;

    const uint8_t swap0_slot = swap_slot_start;
    const uint8_t swap1_slot = swap0_slot + 1;
    const uint8_t swap2_slot = swap1_slot + 1;

    const size_t C_ptr = mem_ptr;

    for (const auto& c : C)
    {
        res += mstore(mem_ptr, push(c));
        mem_ptr += 32;
    }

    const uint8_t T = 3;
    const size_t ROUNDS_F = 8;
    const size_t ROUNDS_P = 57;

    const uint8_t tmp0_slot = swap_slot_end;
    const uint8_t tmp1_slot = tmp0_slot + 1;

    for (size_t r = 0; r < ROUNDS_F + ROUNDS_P; ++r)
    {
        res += storex(T, C_ptr + r * T * 32, state0_slot);
        res += addmodx(state0_slot, swap0_slot, state0_slot);
        res += addmodx(state1_slot, swap1_slot, state1_slot);
        res += addmodx(state2_slot, swap2_slot, state2_slot);

        POW5MODX(state0_slot, tmp0_slot)
        if (r < ROUNDS_F / 2 || r >= ROUNDS_F / 2 + ROUNDS_P)
        {
            POW5MODX(state1_slot, tmp0_slot) POW5MODX(state2_slot, tmp0_slot)
        }

        res += mulmodx(swap0_slot, state0_slot, M00_slot);
        res += mulmodx(swap1_slot, state1_slot, M10_slot);
        res += mulmodx(swap2_slot, state2_slot, M20_slot);

        res += addmodx(swap0_slot, swap0_slot, swap1_slot);
        res += addmodx(swap0_slot, swap0_slot, swap2_slot);

        res += mulmodx(swap1_slot, state0_slot, M01_slot);
        res += mulmodx(swap2_slot, state1_slot, M11_slot);
        res += mulmodx(tmp0_slot, state2_slot, M21_slot);

        res += addmodx(swap1_slot, swap1_slot, swap2_slot);
        res += addmodx(swap1_slot, swap1_slot, tmp0_slot);

        res += mulmodx(swap2_slot, state0_slot, M02_slot);
        res += mulmodx(tmp0_slot, state1_slot, M12_slot);
        res += mulmodx(tmp1_slot, state2_slot, M22_slot);

        res += addmodx(swap2_slot, swap2_slot, tmp0_slot);
        res += addmodx(swap2_slot, swap2_slot, tmp1_slot);
    }

    res += loadx(1, swap0_slot, 0);
    res += ret(0, 32);
    return res;
}

bytecode create_poseidon_hash_bytecode_datacopy()
{
    bytecode res;
    size_t mem_ptr = 0;
    ...
    for (const auto& v : M)
        data += bytecode(intx::be::store<evmc::bytes32>(v).bytes, 32);
    const auto M_size = data.size();

    const auto C_data_offset = M_size;
    for (const auto& v : C)
        data += bytecode(intx::be::store<evmc::bytes32>(v).bytes, 32);
    const auto C_size = data.size() - C_data_offset;
    ...
    const size_t C_ptr = mem_ptr;
    res += datacopy(C_ptr, C_data_offset, C_size);
    ...
    return eof_bytecode(res, 3).data(data);
}

bytecode create_poseidon_hash_bytecode_vectorized()
{
    bytecode res;
    size_t mem_ptr = 0;
    ...
    for (size_t i = 0; i < 9; ++i)
    {
        res += mstore(mem_ptr, push(M[(i * 3) % 9 + i / 3]));
        mem_ptr += 32;
    }

    const uint8_t mds_slot_start = 0;
    const uint8_t M00_slot = mds_slot_start;
    const uint8_t M01_slot = M00_slot + 3;
    const uint8_t M02_slot = M01_slot + 3;
    ...
    {
        res += storex(T, C_ptr + r * T * 32, state0_slot);
        res += addmodx(state0_slot, 1, swap0_slot, 1, state0_slot, 1, T);
        ...
        res += mulmodx(swap0_slot, 1, state0_slot, 1, M00_slot, 1, T);
        res += addmodx(swap0_slot, swap0_slot, swap1_slot);
        res += addmodx(swap0_slot, swap0_slot, swap2_slot);

        res += mulmodx(swap1_slot, 1, state0_slot, 1, M01_slot, 1, T);
        res += addmodx(swap1_slot, swap1_slot, swap2_slot);
        res += addmodx(swap1_slot, swap1_slot, tmp0_slot);

        res += mulmodx(swap2_slot, 1, state0_slot, 1, M02_slot, 1, T);
        res += addmodx(swap2_slot, swap2_slot, tmp0_slot);
        res += addmodx(swap2_slot, swap2_slot, tmp1_slot);
    }
...
bytecode create_poseidon_hash_bytecode_vectorized_datacopy()
{
    bytecode res;
    size_t mem_ptr = 0;
