...
TEST(evmmax, bls12_inv)
{
    ...
    const auto _5_inv_mont = field_inv(s, _5_mont);

    const auto m = s.mul(_5_mont, _5_inv_mont);

    EXPECT_EQ(m, s.to_mont(1));
}
...
// Taken from
// https://github.com/matter-labs/eip1962/blob/master/src/test/test_vectors/eip2537/g1_add.csv#L2
