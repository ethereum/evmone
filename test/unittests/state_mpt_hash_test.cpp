...
{
    EXPECT_EQ(mpt_hash(std::unordered_map<evmone::address, AccountBase>()), emptyMPTHash);
}
...
TEST(state_mpt_hash, single_account_v1)
{
    ...
    AccountBase acc;
    acc.balance = 1_u256;
    const std::unordered_map<address, AccountBase> accounts{{0x02_address, acc}};
    EXPECT_EQ(mpt_hash(accounts), expected);
}

TEST(state_mpt_hash, two_accounts)
{
    std::unordered_map<address, AccountBase> accounts;
...
{
    AccountBase acc;
    // FIXME: acc.storage[0x01_bytes32] = {};
    acc.storage[0x02_bytes32] = {0xfd_bytes32};
    // acc.storage[0x03_bytes32] = {};
    const std::unordered_map<address, AccountBase> accounts{{0x07_address, acc}};
    EXPECT_EQ(mpt_hash(accounts),
