...
TEST(eof, is_eof_code)
{
    ...
    EXPECT_TRUE(is_eof_code("EF00 01 010004 0200010001 00 00000000 00"_hex));
    EXPECT_TRUE(is_eof_code("EF00 01 010004 0200010001 030004 00 00000000 00 AABBCCDD"_hex));
...
TEST(eof, read_valid_eof1_header)
{
    ...
    const TestCase test_cases[] = {
        {"EF00 01 010004 0200010001 00 00000000 00", 4, 1, 0},
        {"EF00 01 010004 0200010006 00 00000400 600160005500", 4, 6, 0},
        ...
        {"EF00 01 010004 0200010100 031000 00 00000000" + std::string(256, '0') + std::string(4096, 'F'), 4, 256, 4096},
    };

    for (const auto& test_case : test_cases)
    {
        const auto code = from_spaced_hex(test_case.code).value();
        const auto header = read_valid_eof1_header(bytes_view(code));
