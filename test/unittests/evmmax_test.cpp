...
inline bytecode create_test_bytecode()
{
    ...
    return calldatacopy(push(0), push(0), push(size * 3)) + setmodx(3, size, 0) +
...
TEST_P(evm, exec_power_of_two_test)
{
    using namespace evmone::test;
    ...
    uint8_t calldata[3 * sizeof(uint256)];
    const auto mod = uint256{1} << 253;
    intx::be::unsafe::store(&calldata[0], mod);
    ...
    for (const auto& x : values)
    {
        for (const auto& y : values)
        {
            const auto expected = umul(x, y) & (mod - 1);
