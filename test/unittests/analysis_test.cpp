...
TEST(analysis, example1)
{
    ...
    const auto analysis = analyze(rev, &code[0], code.size(), OP_STOP);
...
TEST(analysis, jumpdest_skip)
{
    ...
    auto analysis = evmone::analyze(rev, &code[0], code.size(), OP_STOP);
...
TEST(analysis, example1_eof1)
{
    ...
    const auto analysis = analyze(rev, &code[header.code_begin()], header.code_size, OP_INVALID);
