...
struct EXMMAXModState : public EXMMAXModStateInterface
{
    ...
    {
        if (std::max(dst_idx + dst_stride * count,
                std::max(x_idx + x_stride * count, y_idx + y_stride * count)) >= values.size())
            return false;

        std::vector<UintT> temp;
        temp.reserve(count);

        for (size_t i = 0; i < count; ++i)
            temp[i] = op(values[x_idx + i * x_stride], values[y_idx + i * y_stride]);

        for (size_t i = 0; i < count; ++i)
            values[dst_idx + i * dst_stride] = temp[i];
        ...
        size_t y_idx, size_t y_stride, size_t count) noexcept override
    {
        ...
        return operation(std::bind(&ModArith<UintT>::add, arith, _1, _2), dst_idx, dst_stride,
            x_idx, x_stride, y_idx, y_stride, count);
    }
    ...
    {
        using namespace std::placeholders;
        return operation(std::bind(&ModArith<UintT>::sub, arith, _1, _2), dst_idx, dst_stride,
            x_idx, x_stride, y_idx, y_stride, count);
    }
    ...
    {
        using namespace std::placeholders;
        return operation(std::bind(&ModArith<UintT>::mul, arith, _1, _2), dst_idx, dst_stride,
            x_idx, x_stride, y_idx, y_stride, count);
    }
