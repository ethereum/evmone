...
evmc_status_code call_impl(uint256* stack, ExecutionState& state) noexcept
{
    ...
    const auto gas = *stack--;
    const auto dst = intx::be::trunc<evmc::address>(*stack--);
    const auto value = (Op == OP_STATICCALL || Op == OP_DELEGATECALL) ? 0 : *stack--;
    const auto has_value = value != 0;
    const auto input_offset = *stack--;
    const auto input_size = *stack--;
    const auto output_offset = *stack--;
    const auto output_size = *stack;

    *stack = 0;  // Assume failure.
    ...
    *stack = result.status_code == EVMC_SUCCESS;
...
template evmc_status_code call_impl<OP_CALL>(uint256* stack, ExecutionState& state) noexcept;
template evmc_status_code call_impl<OP_STATICCALL>(uint256* stack, ExecutionState& state) noexcept;
template evmc_status_code call_impl<OP_DELEGATECALL>(
    uint256* stack, ExecutionState& state) noexcept;
template evmc_status_code call_impl<OP_CALLCODE>(uint256* stack, ExecutionState& state) noexcept;
...
evmc_status_code create_impl(uint256* stack, ExecutionState& state) noexcept
{
    ...
    const auto endowment = *stack--;
    const auto init_code_offset = *stack--;
    const auto init_code_size = *stack;
    ...
    *stack = 0;
    ...
    if (result.status_code == EVMC_SUCCESS)
        *stack = intx::be::load<uint256>(result.create_address);

    return EVMC_SUCCESS;
}

template evmc_status_code create_impl<OP_CREATE>(uint256* stack, ExecutionState& state) noexcept;
template evmc_status_code create_impl<OP_CREATE2>(uint256* stack, ExecutionState& state) noexcept;
}  // namespace evmone::instr::core
