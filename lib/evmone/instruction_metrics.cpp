...
constexpr metrics_table build_frontier_metrics() noexcept
{
    ...
    metrics[OP_ADDMOD] = {8, 3, -2};
    metrics[OP_MULMOD] = {8, 3, -2};
    ...
    metrics[OP_POP] = {2, 1, -1};
    metrics[OP_MLOAD] = {3, 1, 0};
    metrics[OP_MSTORE] = {3, 2, -2};
    metrics[OP_MSTORE8] = {3, 2, -2};
    metrics[OP_SLOAD] = {50, 1, 0};
    metrics[OP_SSTORE] = {0, 2, -2};
    metrics[OP_JUMP] = {8, 1, -1};
    metrics[OP_JUMPI] = {10, 2, -2};
    metrics[OP_PC] = {2, 0, 1};
    ...
    for (size_t op = OP_DUP1; op <= OP_DUP16; ++op)
        metrics[op] = {3, int8_t(op - OP_DUP1 + 1), 1};

    for (size_t op = OP_SWAP1; op <= OP_SWAP16; ++op)
        metrics[op] = {3, int8_t(op - OP_SWAP1 + 2), 0};

    // FIXME: Use loop?
    metrics[OP_LOG0] = {1 * 375, 2, -2};
    metrics[OP_LOG1] = {2 * 375, 3, -3};
    metrics[OP_LOG2] = {3 * 375, 4, -4};
    metrics[OP_LOG3] = {4 * 375, 5, -5};
    metrics[OP_LOG4] = {5 * 375, 6, -6};

    metrics[OP_CREATE] = {32000, 3, -2};
    metrics[OP_CALL] = {40, 7, -6};
    metrics[OP_CALLCODE] = {40, 7, -6};
    metrics[OP_RETURN] = {0, 2, -2};
    metrics[OP_INVALID] = {0, 0, 0};
    metrics[OP_SELFDESTRUCT] = {0, 1, -1};
...
constexpr metrics_table build_homestead_metrics() noexcept
{
    ...
    metrics[OP_DELEGATECALL] = {40, 6, -5};
    return metrics;
}
...
constexpr metrics_table build_byzantium_metrics() noexcept
{
    ...
    metrics[OP_RETURNDATACOPY] = {3, 3, -3};
    metrics[OP_STATICCALL] = {700, 6, -5};
    metrics[OP_REVERT] = {0, 2, -2};
    return metrics;
}
...
constexpr metrics_table build_constantinople_metrics() noexcept
{
    ...
    metrics[OP_EXTCODEHASH] = {400, 1, 0};
    metrics[OP_CREATE2] = {32000, 4, -3};
    return metrics;
}
}  // namespace
