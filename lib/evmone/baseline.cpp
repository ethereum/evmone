...
CodeAnalysis analyze_eof2(const uint8_t* code, const EOF2Header& header)
{
    ...
    // Using "raw" new operator instead of std::make_unique() to get uninitialized array.
    std::unique_ptr<uint8_t[]> padded_code{new uint8_t[header.code_size + code_padding]};
    std::copy_n(code + header.code_begin(), header.code_size, padded_code.get());
...
CodeAnalysis analyze(evmc_revision rev, const uint8_t* code, size_t code_size)
{
    ...
    return analyze_eof2(code, eof2_header);
}
