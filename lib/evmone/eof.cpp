    ...
    bytes_view code, size_t func_index, const std::vector<EOFCodeType>& code_types)
{
    ...
    while (!worklist.empty())
    {
        ...
        assert(stack_height_min != LOC_UNVISITED);
        ...
        const auto validate_successor = [&stack_heights_min, &stack_heights_max, &worklist](
                                            size_t current_offset, size_t successor_offset,
                                            int32_t expected_stack_height_min,
                                            int32_t expected_stack_height_max) {
            ...
            {
                if (successor_stack_height_min == LOC_UNVISITED)
                {
            ...
            else  // forwards jump, target known
            {
                ...
                // THIS LEADS TO RE-VALIDATION OF CODE BLOCK, MAKING COMPLEXITY QUADRATIC
                worklist.push(successor_offset);
                return true;
            }
        };
    ...
    if (std::find(stack_heights_min.begin(), stack_heights_min.end(), LOC_UNVISITED) !=
        stack_heights_min.end())
