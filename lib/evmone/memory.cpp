...
const bool memory_initialized = []() noexcept
{
...
size_t map_gas_to_memory_ceiling(int64_t available_gas) noexcept
{
    ...
    size_t max_tx_words = std::sqrt(static_cast<long double>(
                              2359296 + 2048 * static_cast<long double>(available_gas))) /
                          2;
    ...
    // This in turn makes it possible to defer gas accounting for memory reads/writes, as it is no
    // longer possible to write to memory that is out of bounds. However...performing gas accounting
    // per basic block was less performant than just performing the computation when required.
    // (maybe using this in conjunction with running gas accounting in a second, non-blocking thread
    // would be faster?) size_t leading_zeroes = 0; if (sizeof(size_t) == sizeof(unsigned long
    // long))
    // {
...
void clean_up(size_t msize) noexcept
{
    ...
    else
    {
        ...
        // current_memory persists beyond the scope of this function, so the compiler shouldn't
        // optimize this out!
        memset(current_memory_page.pointer + current_memory_page.allocated_memory, 0,
