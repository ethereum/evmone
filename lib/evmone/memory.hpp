...
uint8_t* get_tx_memory_ptr(size_t available_gas) noexcept
{
    ...
    size_t max_tx_words =
        std::sqrt(static_cast<double>(2359296 + 2048 * static_cast<double>(available_gas))) / 2;
    size_t max_tx_memory = (max_tx_words * 32);

    // We want the amount of allocated memory - 32 to be equal to a power of (2^n - 1).
    // This is so that, for memory opcodes, we can 'pin' the maximum memory index with a simple
    // logical AND
    size_t tx_allocated_memory =
        static_cast<size_t>((static_cast<unsigned int>(~0) >> __builtin_clz(max_tx_memory)) + 32);
...
void clean_up(size_t msize) noexcept
{
    ...
    {
        // the current memory page is still the latest. We need to zero out the relevant amount of
        // memory
        memory_page& current_memory_page = *current_memory;
