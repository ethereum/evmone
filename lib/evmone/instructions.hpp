...
inline Result setmodx(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    const auto mod_size_bytes = (mod_size_256 + 7) / 8;
    const auto vals_used_256 = stack.pop();

    if (!check_memory(gas_left, state.memory, mod_offset_256, mod_size_bytes))
        return {EVMC_OUT_OF_GAS, gas_left};

    // Maximum allowed modulus size
    if (mod_size_bytes > MAX_MOD_SIZE / 8)
    ...
    // Modulus must be odd
    if (mod_ptr[static_cast<size_t>(mod_size_bytes) - 1] % 2 == 0)
        return {EVMC_FAILURE, gas_left};
    ...
    const auto value_size_multiplier = (mod_size + 7) / (8 * 8);
...
inline Result loadx(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    if (!check_memory(gas_left, state.memory, dst_offset,
            num_vals * state.evmmax_state.active_mod_value_size()))
        return {EVMC_OUT_OF_GAS, gas_left};
...
inline Result storex(StackTop stack, int64_t gas_left, ExecutionState& state) noexcept
{
    ...
    if (!check_memory(
            gas_left, state.memory, offset, num_vals * state.evmmax_state.active_mod_value_size()))
        return {EVMC_OUT_OF_GAS, gas_left};
