...
// Algorithm 31
// TODO: Use squaring algorithm for Fq12 from mentioned paper. (Algorithm 24)
    ...
    const std::vector<std::array<uint256, 4>>& vG2, const std::vector<Point>& vG1) noexcept
{
    ...
    for (size_t i = 0; i < vG2.size(); ++i)
    {
        if (!is_field_element(vG1[i].x) || !is_field_element(vG1[i].y) ||
            !is_field_element(vG2[i][0]) || !is_field_element(vG2[i][1]) ||
            !is_field_element(vG2[i][2]) || !is_field_element(vG2[i][3]))
        {
            return std::nullopt;
        }

        // Coverts points' coefficients in Montgomery form.
        const auto Q_aff = ecc::Point<Fq2>(Fq2({Fq::from_int(vG2[i][0]), Fq::from_int(vG2[i][1])}),
            Fq2({Fq::from_int(vG2[i][2]), Fq::from_int(vG2[i][3])}));
