...
namespace constants
{
inline constexpr auto _1 = Fq::from_int(1);
inline constexpr auto _2 = Fq::from_int(2);
inline constexpr auto _3 = Fq::from_int(3);
inline constexpr auto _4 = Fq::from_int(4);
inline constexpr auto _8 = Fq::from_int(8);
inline constexpr auto _9 = Fq::from_int(9);

namespace
{
inline constexpr Fq2 make_fq2(std::array<uint256, 2> a) noexcept
{
    return Fq2({Fq::from_int(a[0]), Fq::from_int(a[1])});
}
}  // namespace
...
template <typename ValueT>
std::ostream& operator<<(std::ostream& out, const ecc::JacPoint<ValueT>& p)
{
    out << "x: " + p.x.to_string() << std::endl;
    out << "y: " + p.y.to_string() << std::endl;
    out << "z: " + p.z.to_string() << std::endl;
...
template <typename ValueT>
std::ostream& operator<<(std::ostream& out, const ecc::Point<ValueT>& p)
{
    out << "x: " + p.x.to_string() << std::endl;
    out << "y: " + p.y.to_string() << std::endl;
...
inline constexpr bool is_field_element(const uint256& v)
{
    return v < FieldPrime;
}

// Verifies that affine point is on the curve (not twisted)
inline constexpr bool is_on_curve(const evmmax::ecc::Point<Fq>& p)
{
...
inline constexpr bool is_on_twisted_curve(const evmmax::ecc::Point<Fq2>& p)
{
    using namespace constants;
    const auto x3 = p.x * p.x * p.x;
    const auto y2 = p.y * p.y;

    return y2 == x3 + _3 * Fq6Config::ksi.inv();
}

// Verifies that affine point over the base field is infinity.
inline constexpr bool is_infinity(const evmmax::ecc::Point<Fq>& p)
{
    return p.x.is_zero() && p.y.is_zero();
}

// Verifies that affine point over the Fq^2 extended field is infinity.
inline constexpr bool g2_is_infinity(const evmmax::ecc::Point<Fq2>& p)
{
...
// TODO: add reference that it's exacly the same as untwist->frobenius->twist
template <uint8_t P>
inline constexpr typename std::enable_if<P == 1 || P == 3, ecc::JacPoint<Fq2>>::type endomorphism(
    const ecc::JacPoint<Fq2>& p) noexcept
{
    auto r = ecc::JacPoint<Fq2>(p.x.conjugate(), p.y.conjugate(), p.z.conjugate());

    r.x = r.x * constants::FROBENIUS_COEFFS[P - 1][1];
    r.y = r.y * constants::FROBENIUS_COEFFS[P - 1][2];
...
template <uint8_t P>
inline constexpr typename std::enable_if<P == 2, ecc::JacPoint<Fq2>>::type endomorphism(
    const ecc::JacPoint<Fq2>& p) noexcept
{
    auto r = ecc::JacPoint<Fq2>(p.x, p.y, p.z);

    r.x = r.x * constants::FROBENIUS_COEFFS[1][1];
    r.y = r.y * constants::FROBENIUS_COEFFS[1][2];

    return r;
}

template <uint8_t P>
inline constexpr std::enable_if<P == 1 || P == 3, ecc::Point<Fq2>>::type endomorphism(
    const ecc::Point<Fq2>& p) noexcept
{
    auto r = ecc::Point<Fq2>(p.x.conjugate(), p.y.conjugate());
...
template <uint8_t P>
inline constexpr typename std::enable_if<P == 2, ecc::Point<Fq2>>::type endomorphism(
    const ecc::Point<Fq2>& p) noexcept
{
    return {p.x * constants::FROBENIUS_COEFFS[1][1], p.y * constants::FROBENIUS_COEFFS[1][2]};
}

// Computes Frobenius endomorphism for Fq12 field member values
// This specialisation computes Frobenius and Frobenius^3
template <uint8_t P>
inline constexpr std::enable_if<P == 1 || P == 3, Fq12>::type endomorphism(const Fq12& f) noexcept
{
    using namespace constants;
    return Fq12({
        Fq6({
...
// This specialiations computes Frobenius^2
template <uint8_t P>
inline constexpr typename std::enable_if<P == 2, Fq12>::type endomorphism(const Fq12& f) noexcept
{
    using namespace constants;
    return Fq12({
        Fq6({
...
inline constexpr ecc::JacPoint<Fq2> add(
    const ecc::JacPoint<Fq2>& P0, const ecc::JacPoint<Fq2>& P1) noexcept
{
    ...
    const auto X3 = R_squared - H_cubed - _2 * U1 * H_squared;
...
inline constexpr ecc::JacPoint<Fq2> dbl(const ecc::JacPoint<Fq2>& Q) noexcept
{
    using namespace constants;
    ...
    const auto R = _2 * y_squared;
    const auto S = _2 * x * R;
    const auto M = _3 * x_squared;

    const auto Xp = M * M - _2 * S;
    const auto Yp = M * (S - Xp) - _8 * y_4;
    const auto Zp = _2 * y * z;
...
template <int N>
inline constexpr ecc::JacPoint<Fq2> n_dbl(const ecc::JacPoint<Fq2>& a) noexcept
{
    auto r = dbl(a);
    for (int i = 0; i < N - 1; ++i)
        r = dbl(r);
...
// Addchain generated algorithm which multiplies point `a` in Jacobian coordinated by X (curve seed)
inline constexpr ecc::JacPoint<Fq2> mul_by_X(const ecc::JacPoint<Fq2>& a) noexcept
{
    auto t0 = dbl(a);
    auto t2 = dbl(t0);
    auto c = dbl(t2);
...
inline constexpr bool g2_subgroup_check(const evmmax::ecc::Point<Fq2>& p_aff)
{
    const auto p = evmmax::ecc::JacPoint<Fq2>::from(p_aff);
...
// TODO: Add a reference to the the formula deriviation
inline constexpr ecc::JacPoint<Fq2> lin_func_and_dbl(
...
// Computes points P0 and P1 addition for twisted curve + line defined by untwisted P1 and P2 points
// on the curve (not twisted curve) evaluated at point P. Formula is simplyfied for P1.z == 1.
// TODO: Add a reference to the the formula deriviation
[[nodiscard]] inline constexpr ecc::JacPoint<Fq2> lin_func_and_add(
    const ecc::JacPoint<Fq2>& P0, const ecc::Point<Fq2>& P1, std::array<Fq2, 3>& t) noexcept
{
    ...
    const auto X3 = R_squared - H_cubed - _2 * x0 * H_squared;
...
// on the curve (not twisted curve) evaluated at point P. Formula is simplyfied for P1.z == 1.
// TODO: Add a reference to the the formula deriviation
inline constexpr void lin_func(
    const ecc::JacPoint<Fq2>& P0, const ecc::Point<Fq2>& P1, std::array<Fq2, 3>& t) noexcept
{
    using namespace constants;
...
[[nodiscard]] inline constexpr Fq12 square(const Fq12& f) noexcept
{
...
inline constexpr std::pair<Fq2, Fq2> fq4_square(const std::pair<Fq2, Fq2>& a)
{
...
inline constexpr Fq12 cyclotomic_square(const Fq12& c)
{
...
template <int N>
inline constexpr Fq12 n_cyclotomic_square(const Fq12& c)
{
    auto r = c;
    for (int i = 0; i < N; ++i)
...
inline constexpr Fq12 cyclotomic_pow_to_X(const Fq12& a)
{
