...
// This specialisation computes Frobenius and Frobenius^3
template <uint8_t P>
inline constexpr typename std::enable_if<P == 1 || P == 3, ecc::Point<Fq2>>::type endomorphism(
...
// This specialisation computes Frobenius and Frobenius^3
template <uint8_t P>
inline constexpr typename std::enable_if<P == 1 || P == 3, Fq12>::type endomorphism(
    const Fq12& f) noexcept
{
    using namespace constants;
    return Fq12({
        Fq6({
...
// on the curve (not twisted curve) evaluated at point P. Formula is simplified for P1.z == 1.
// For more details see https://notes.ethereum.org/@ipsilon/Hkn2a2qk0
...
// Computes `cyclotomic_square` n times. It is a helper function.
template <int N>
...
// Computes `a^X` where `X` is the curve seed parameter and `a` is from cyclotomic subgroup of Fq^12
