...
struct BaseFieldConfig
{
    ...
    static constexpr ModArith<ValueT> MOD_ARITH =
        ModArith(FieldPrime, 0x6d89f71cab8351f47ab1eff0a417ff6b5e71911d44501fbf32cfc5b538afa89_u256,
            9786893198990664585u);
...
struct Fq6Config
{
    ...
    static constexpr Fq2 ksi = Fq2({Fq::from_int(9_u256), Fq::from_int(1_u256)});
};
...
inline constexpr Fq2 multiply(const Fq2& a, const Fq2& b)
{
    return Fq2({
...
inline constexpr Fq6 multiply(const Fq6& a, const Fq6& b)
{
...
inline constexpr Fq12 multiply(const Fq12& a, const Fq12& b)
{
    const auto& a0 = a.coeffs[0];
    const auto& a1 = a.coeffs[1];
    const auto& b0 = b.coeffs[0];
...
inline constexpr Fq6 inverse(const Fq6& f)
{
    const auto& a0 = f.coeffs[0];
    const auto& a1 = f.coeffs[1];
...
inline constexpr Fq12 inverse(const Fq12& f)
{
    const auto& a0 = f.coeffs[0];
    const auto& a1 = f.coeffs[1];
