...
public:
    ...
    [[nodiscard]] uint384 inv(const uint384& x) const noexcept;
...
//    uint384 neg(const uint384& x) const noexcept;
    ...
    template <size_t N>
    [[nodiscard]] uint384 in_mont() const noexcept
    {
        static const auto n_value = to_mont(N);
        return n_value;
    }

    [[nodiscard]] uint384 one_mont() const noexcept { return in_mont<1>(); }
