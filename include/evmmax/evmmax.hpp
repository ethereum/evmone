...
public:
    ...
    /// Compute the modular inversion of the x in Montomery form. The result is in Montgomery form.
    constexpr UintT inv(const UintT& x) const noexcept
    {
        assert((mod & 1) == 1);
        assert(mod >= 3);

        // Precompute ½ % mod as (mod + 1) / 2.
        ...
        while (a != 0)
        {
            ...
            // Compute a / 2 % mod, a is even so division is exact.
            a >>= 1;
